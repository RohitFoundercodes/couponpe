...
public function getQuote(Request $request) {
    try {
       ...
       $insert=DB::table('getQuote')->insert(['txn_id' =>$txn_id,'userid' => $userid,'quote'=>$quote,'iv'=>$iv,'item_id'=>$item_id,'company_id'=>$companyOrderId,'itemname'=>$itemname,'logo'=>$logo,'amount'=>$amount,'discount'=>0]);
        return [
            'success' => true,
            'message' => 'GetQuote retrieved successfully.',
    ...
    try {
        ...
        if ($a === null) {
         ...
         if ($encryptedData->status->code == "200") {
            return response()->json(['success' => true, 'message' => 'Successfully', 'data'=>$encryptedData]);
        } else {
